...
<%--                 <option value="writer"  <c:if test="${search=='writer'}">selected="selected" </c:if>>작성자</option> --%>
        ...
        <c:forEach var="b" items="${boardlist}">
		    <c:if test="${b.anState == 1}">
		        <tr>
		            ...
		            <td>
		                <a href="announcementContent?no=${b.anNum}&page=${page}&state=${b.anState}">
		                    ${b.anTitle}
		                </a>
		            </td>
		            <td>관리자</td>
		            <td>
		                <fmt:formatDate value="${b.anDate}" pattern="yyyy-MM-dd HH:mm:ss" />
		            </td>
		        </tr>
		    </c:if>
		</c:forEach>
    ...
    <div style="text-align: center">
        <c:if test="${listcount>0}">
            ...
            <!-- 마지막 페이지로 이동 -->
            <a href="announcement?page=${pagecount}" style="text-decoration: none"> > </a> 
        </c:if>
    </div>
